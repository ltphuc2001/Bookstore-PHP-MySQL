...
class Validate
{
            ...
            if ($value['required'] == true && trim($this->source[$element]) == null) {
                ...
                switch ($value['type']) {
                    ...
                    case 'status':
                        ...
                        break;
                    case 'select':
                        $this->validateSelect($element, $value['options']);
                        break;
                    case 'group':
    ...
    public function showErrorsBackend()
    {
        $xhtml = '';
        if (!empty($this->errors)) {
            $xhtml .= '
			...
			';
            foreach ($this->errors as $key => $value) {
                $xhtml .= sprintf('<li class="text-white">%s</li>', $value);
            }
            $xhtml .=  '</ul></div>';
        }
        return $xhtml;
    ...
    public function showErrorsPublic()
    {
        ...
        if (!empty($this->errors)) {
			...
			';
            foreach ($this->errors as $key => $value) {
                $xhtml .= sprintf('<li class="text-white">%s</li><br>', $value);
            }
            $xhtml .=  '</ul></div>';
        }
        return $xhtml;
    ...
    private function validateSelect($element, $options)
    {
        if (in_array($this->source[$element], $options)) {
            $this->setError($element, 'Giá trị đã chọn không hợp lệ!');
        }
    }
    ...
    private function validateExistRecord($element, $options)
    {
        ...
        if ($database->isExist($query) == false) {
            $this->setError($element, 'giá trị này không hợp lệ!');
        }
    }
